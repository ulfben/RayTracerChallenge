...
constexpr Matrix4 operator*(const Matrix4& lhs, const Real& s) noexcept {
    return {
        lhs[0] * s, lhs[1] * s, lhs[2] * s, lhs[3] * s,
        lhs[4] * s, lhs[5] * s, lhs[6] * s, lhs[7] * s,
        lhs[8] * s, lhs[9] * s, lhs[10] * s, lhs[11] * s,
        lhs[12] * s, lhs[13] * s, lhs[14] * s, lhs[15] * s
    };
}
...
/*constexpr*/ Matrix4 rotation(Vector axisOfRotation, Real angle) noexcept {    
    ...
    const auto s_kx = s * kx;
    const auto s_ky = s * ky;
    const auto s_kz = s * kz;

    return Matrix4{
        cos_t + s_kx * kx,          -sin_t * kz + s_kx * ky,    sin_t * ky + s_kx * kz,     0,
        sin_t * kz + s_ky * kx,     cos_t + s_ky * ky,          -sin_t * kx + s_ky * kz,    0,
        -sin_t * ky + s_kz * kx,    sin_t * kx + s_kz * ky,     cos_t + s_kz * kz,          0,
        0,                          0,                          0,                          1
    };
}
